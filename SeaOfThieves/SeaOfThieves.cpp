...
void SeaOfThieves::Initialise()
{
	...
	spdlog::set_default_logger(spdlog::basic_logger_mt("SeaOfThieves", tmpDir + "SeaOfThieves", true));
#if _DEBUG == TRUE
	spdlog::set_level(spdlog::level::debug);
#endif

	// Log the initialisation
	spdlog::info("Initialising SeaOfThieves");
	...
	if (NullChecks())
	{
		...
		if (playerController)
		{
			...
			/*if (playerController->WasInputKeyJustReleased(FKey("End")))
			{
				...
				HMODULE hMod = GetModuleHandle(L"SeaOfThieves.dll");
